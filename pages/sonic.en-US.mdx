...
1) Unstake their sFTMx using the "Unstake" tab on the [Fantom sFTMx UI](https://ftm.beets.fi/sftmx).*

2) Wait until the 24 hour unstaking period concludes and then withdraw the FTM using the "Withdraw" tab on the [sFTMx UI](https://ftm.beets.fi/sftmx).*

3) Exchange wFTM for S (Sonic's native token) through [Sonic Labs' bridge](https://my.soniclabs.com/upgrade).

4) Stake S for stS using the (stS UI)[https://beets.fi/stake].
...
1) Break maBEETS position on Fantom to withdraw the proportional amounts of wFTM and BEETS using the [FTM maBEETS UI](https://ftm.beets.fi/mabeets).
...
4) Stake S for stS using the stS UI. (link TBA)

5) Recreate the maBEETS position by pairing the bridged BEETS with stS using the [Sonic maBEETS UI](https://ma.beets.fi/).
