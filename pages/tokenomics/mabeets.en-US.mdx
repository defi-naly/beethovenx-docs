# maBEETS
Maturity Adjusted BEETS (maBEETS) empowers users to shape the protocol’s evolution through decentralized governance while earning a share of all protocol revenue. Unlike the traditional vote escrow (ve) model, where voting power depends on lock duration, maBEETS scales voting power and rewards dependent on how long a position is held. Users are never forcibly locked and can exit their positions at any time.
...
When users create a maBEETS position, they receive a financial NFT called a Relic, which serves as a receipt for their position. This NFT dynamically tracks key details such as position size, maturity, voting power, and reward distribution. As a result, maBEETS offers several unique benefits:
